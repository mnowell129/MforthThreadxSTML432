                   ...
                   int precision, int flags)
{
   ...
   for(strln = 0;value[strln] != '\0' &&
       (noprecision || strln < precision);strln++) continue;
       ...
       int precision, int flags)
{
   UINTMAX_T uvalue;
   /**
    * CMN arrays are static, not reentrant, but stack depth is 
    * controlled. 
    */
   static char iconvert[MAX_CONVERT_LENGTH];
   static char sign = 0;
   static char hexprefix = 0;
   static int spadlen = 0; /* Amount to space pad. */
   static int zpadlen = 0; /* Amount to zero pad. */
   static int pos;
   static int separators;
   static int noprecision;

   sign = 0;
   hexprefix = 0;
   spadlen = 0;   /* Amount to space pad. */
   zpadlen = 0;   /* Amount to zero pad. */
...
{
   static LDOUBLE ufvalue;
   static UINTMAX_T intpart;
   static UINTMAX_T fracpart;
   static UINTMAX_T mask;
   static const char *infnan;
   /**
    * CMN : make arrays static to avoid stack depth problems 
    */
   static char iconvert[MAX_CONVERT_LENGTH];
   static char fconvert[MAX_CONVERT_LENGTH];
   static char econvert[4];   /* "e-12" (without nul-termination). */
   static char esign; // = 0;
   static char sign; // = 0;
   static int leadfraczeros; // = 0;
   static int exponent; // = 0;
   static int emitpoint; // = 0;
   static int omitzeros; // = 0;
   static int omitcount; // = 0;
   static int padlen; // = 0;
   static int epos; // = 0;
   static int fpos; // = 0;
   static int ipos; //  = 0;
   static int separators; //  = (flags & PRINT_F_QUOTE);
   static int estyle; //  = (flags & PRINT_F_TYPE_E);
...
again:
    ...
    */
   mask = (UINTMAX_T)mypow10((UINTMAX_T)precision);
   /*
...
void __attribute__((weak)) lockPrintf(void)
{
}
void __attribute__((weak)) unLockPrintf(void)
{
}
/**
 * modified to use statics and 
 * call a lock routine to avoid 
 * re-entrancy issues 
 * 
 * @author nowelc2 (11/8/2017)
 * 
 * @param str 
 * @param size 
 * @param format 
...
{
   static LDOUBLE fvalue;
   static INTMAX_T value;
   static unsigned char cvalue;
   static const char *strvalue;
   static INTMAX_T *intmaxptr;
   static PTRDIFF_T *ptrdiffptr;
   static SSIZE_T *sizeptr;
   static LLONG *llongptr;
   static long int *longptr;
   static int *intptr;
   static short int *shortptr;
   static signed char *charptr;
   /*static */ size_t len; // = 0; // Needs to be caller specific
   ...
   static int state; // = PRINT_S_DEFAULT;
   static char ch; //  = *format++;
...
#else
                  /**
                   * cmn : make this truly take a float without conversion
                   */
                  fvalue = *(float *)(args._Ap);
                  args._Ap += sizeof(float);
#endif
               }
...
int rpl_vsnprintf(char *str, size_t size, const char *format, ...)
{
   ...
   _rpl_vsnprintf(&(addedLength),str,size,format,list);
   va_end(list);
   return(addedLength);
}
